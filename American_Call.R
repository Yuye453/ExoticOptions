...
p = 1/2     # Probability of an increment in the stock price
      ...
      # Function
      american_call_option = function(S, E, T, r, sigma, M, p) {
        ...
        for (i in (M-1):0) {
          ...
          W = pmax(max(S_matrix[i+1] - (Unitary * E), 0), exp(-r*delta_t) * ((1-p)*W[1:(i+1)] + p*W[2:(i+2)]))
...
# Example
american_call_option(S = 100, E = 120, T = 1, r = 0.05, sigma = 0.3, M = 50, p = 1/2)